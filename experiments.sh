#!/bin/bash
# Run many experiments to find the fastest mosey configuration
...
# Pre-warm daemon
mosey stop 2>/dev/null; pkill -f "bun.*daemon" 2>/dev/null; rm -f ~/.mosey/daemon.json; sleep 1
mosey --headless https://example.com > /dev/null 2>&1
...
# E1: Current v5 (plain text, mosey <url>)
run_experiment "E1_plaintext" \
  "Use 'mosey <url>' to read any webpage. It returns plain text with [ref=eN] IDs. To click: mosey do e5. To type: mosey do e5 --value 'text'."

# E2: Ultra-minimal prompt
run_experiment "E2_minimal" \
  "mosey <url> reads a webpage."

# E3: Directive prompt (tell model exactly what to run)
run_experiment "E3_directive" \
  "Run mosey <url> to read a page. Answer based on the output. Do not run any other commands unless the task requires clicking."

# E4: JSON output (old style - navigate + state)
run_experiment "E4_json_2step" \
  "You have 'mosey' CLI. Commands: navigate <url> (JSON), state (JSON page content), do <ref> (click). Use navigate then state to read a page."

# === OUTPUT FORMAT EXPERIMENTS ===

# E5: Depth-limited snapshot (depth=4)
run_experiment "E5_depth4" \
  "Run 'mosey navigate <url>' then 'mosey state --json | python3 -c \"import sys,json,requests; r=requests.get(\\\"http://127.0.0.1:\$(cat ~/.mosey/daemon.json | python3 -c \\\"import sys,json;print(json.load(sys.stdin)[\\\\\\\"port\\\\\\\"])\\\")/state?depth=4\\\"); print(r.json()[\\\"snapshot\\\"])\"' — actually just use 'mosey <url>' to read a page."
...
run_experiment "E6_trunc3k" \
  "Run 'mosey <url> | head -c 3000' to read a webpage (truncated for speed). Answer from what you see."

# E7: Only interactive elements (grep for refs)
run_experiment "E7_interactive_only" \
  "Run 'mosey <url> | grep \"\\[e\"' to get interactive elements only. Answer from that."
